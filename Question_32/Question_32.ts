...
let new_users:string[] = ['STEPHEN', 'Mike', 'Jenny', 'Elina', 'Albert']
let current_users:string[] = ['Eric', 'Emily', 'Stephen', 'Mike', 'Lily'];

new_users.forEach((val) =>{
    const taken = current_users.some((item) => item.toLowerCase() === val.toLowerCase())
    if(taken){
        console.log(`username is not avaiable ${val}, try another username.`)
    }
    else{
        console.log(`username avaiable ${val}.`)
    }
})